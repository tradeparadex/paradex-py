...
from paradex_py.message.order import build_order_message, build_order_message_for_modify
...
class ParadexAccount:
    ...
    def sign_order(self, order: Order) -> str:
        if order.id:
            sig = self.starknet.sign_message(build_order_message_for_modify(self.l2_chain_id, order))
        else:
