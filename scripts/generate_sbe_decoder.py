...
def parse_schema(schema_path: str):  # noqa: C901
    ...
    # ElementTree drops comments, and the release date only lives in the
    # header comment, so read it off the raw text rather than hardcoding it.
    released_match = re.search(r"^\s*Released\s*:\s*(\S+)", Path(schema_path).read_text(), re.MULTILINE)
    if released_match is None:
        raise ValueError("Schema header has no 'Released :' line")
            ...
            fields.append(
                {
                    ...
                    # The schema marks a retired field by opening its
                    # description with DEPRECATED; the model carries that
                    # through as a pydantic deprecation.
                    "deprecated": description if description.startswith("DEPRECATED") else None,
                }
            )
...
# ── OrderEvent.request_info ─────────────────────────────────────────────────
#
# The JSON feed nests request_info and omits it on nearly every update; SBE
# flattens it into requestStatus/requestType/requestId/requestMessage, with
...
def generate_codec(schema: dict) -> str:  # noqa: C901
    ...
    lines += [
        ...
        "#   SBE trade_id_str ↔ JSON id  (TradeEvent.trade_id is the deprecated int64)",
...
"""
    ...
    for msg in messages:
        ...
        for f in fields:
            ...
            annotation = _absent_by_version(python_type, f["since"])
            if f["deprecated"]:
                if f["since"] > 0:
    ...
    for msg in messages:
        ...
        #
        # The header version alone is not enough, though: 1:2 gained fields in
        # place after it was first served, so a 1:2 frame from a server that
        # predates them simply ends where the older layout did. An appended
        # field that is absent at the very end of the frame is therefore
        # missing, not malformed; one that starts but runs short still raises.
        for d in data_fields:
            ...
            if d["since"] == 0:
                ...
                lines += [
                    f"    {dname} = None",
                    f"    if version >= {d['since']} and offset < len(payload):",
                    f"        {dname}, offset = _read_str(payload, offset)",
                ]

        if msg_id in _REQUEST_INFO_IDS:
            lines += _REQUEST_INFO_LINES
